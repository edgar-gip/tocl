...
%% Auto-HDS (Hierarchical Density Shaving)
%% From:
...
function [ this ] = AutoHDS(divergence, opts = struct())

  %% Check arguments
  if ~any(nargin() == [ 1, 2 ])
    usage("[ this ] = AutoDS(divergence [, opts])");
  endif
  ...
  %% Particle size
  %% Default -> 3
  this.n_part = getfielddef(opts, "n_part", 3);
  ...
  %% Shaving ratio
  %% Default -> 0.01
  this.r_shave = getfielddef(opts, "r_shave", 0.01);
