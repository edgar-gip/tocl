...
methods = ...
    struct("rproj2", ...
	   ...
	   "hvoro_rbf", ...
	       Voronoi(KernelDistance(RBFKernel(2.0)), ...
		       struct("soft_alpha", nan)), ...
	   "voro_mah", @(data) ...
	       Voronoi(MahalanobisDistance(data), ...
		       struct("soft_alpha", 10.0)), ...
	   "voro_rbf", ...
	       Voronoi(KernelDistance(RBFKernel(10.0)), ...
		       struct("soft_alpha", 10.0)));
           %% "hproj", ...
	   %%     RandomProj(struct("soft_alpha", nan,
	   %% 			 "homogeneous", true())), ...
...
for s = 1 : cmd_opts.n_samples
  ...
  for i = 1 : length(names)
	...
	fprintf(2, "Sample: %-3d Method: %-15s Repeat: %-3d\n", s, name, r);
      ...
      if isfield(method_k_max, met)
	k_max = getfield(method_k_max, met);
      else
	k_max = method_k_max.default;
      endif
      k = floor(2 + (k_max - 1) * rand());
